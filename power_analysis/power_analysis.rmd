...
```{r}
set.seed(1)
n.subject <- seq(from=60, to=180, by=12)
n.per.subject <- 3
n.per.condition <- 1 #LL: needs to be increased 
age.range <- 10:168 #based on Lonardo et al. (2021)
#breed <- c(
...
# Cooperative
coop.breed_fb.per    <- 0.48
coop.breed_tb.per    <- 0.29
#data from Lonardo et al. (2021)
#but also if we considered the results of Lurz et al. (2022),
#they also found a 20% difference
#prop.searches biased towards A in FB= 0.28
#prop.searches biased towards A in TB= 0.08
#so we have no grounds to assume a condition difference larger than 20% and should in fact expect something smaller.
...
# simulations
n.simus <- 1 #LL: revert back to 1000
...
for (j in seq_along(n.subject)) {
  ...
  # all.res$warns.null <- NA #LL: we don't need a null model if we are only interested in condition, right?
  ...
  design <- expand.grid(
    condition   = c("false_belief", "true_belief"),
  #  trial = c(1:3), #LL: needs adjustment. Right now, with 2 conditions it assigns trial 1 to 3 to each condition. In the end I would like to simulate something like 4 trials per each of 3 conditions. In the end we need to decide the sample size on the exact design we will follow otherwise the power analysis is not so useful because a reviewer might still tell us that we were underpowered even to detect the difference between tb and fb.
    sex         = c("F", "M"),
  ...
  # Fixed per subject: age and z.age (compute once) #LL: so trial should be added above because it is not fixed per subejct
  ...
  start.data$condition  <- factor(start.data$condition, levels = c("false_belief","true_belief"))
  ...
  m.mat <- model.matrix(~ condition + sex + baited.fst + z.age, data = start.data) #LL: add trial number?
  LP_fixed <- as.numeric(m.mat[, names(coefs), drop = FALSE] %*% coefs)

  # Export objects to workers (only small/light objects)
  clusterExport(cl, varlist = c("start.data","LP_fixed","r.effects","r.slope.condition","contr","get_p", "coop.breeds"),
                envir = environment())
...
dat$breed <- factor(dat$breed) 
  ...
  #  mc_tb_ind  <- means["true_belief","independent"]
    ...
    fe <- tryCatch(fixef(full$value), error = function(e) setNames(rep(NA_real_, 9),
                     c("(Intercept)","conditiontrue_belief",
    ...
    out <- list(
      ...
      #lrt.p.drop1.breed_type = get_p(d1, "breed_type"),
  ...
  num_cols <- c("icpt","conditiontrue_belief",
                "sexM","baited.fstgrey","z.age",
                "warns.full","re.sd",
                "lrt.p.drop1.condition",  "lrt.p.drop1.sex","lrt.p.drop1.baited_fst","lrt.p.drop1.z_age",
...
save.image(file = here("power_analysis", "power_sim_snuffle_mat_workspace_parallel.RData"))
```
...
lrt.data2 <- all.res2 %>%
  ...
  dplyr::summarise(
    ...
    # proportion.sign.lrt.breed.type = n.sign.lrt.breed.type / n.simus,  
...
lrt.data2
save.image("power_sim_snuffle_mat_eval.RData")
...
plot.sample.sizes.power.snuffle.mat <- ggplot(lrt.data2,
                                              aes(x = n.subject, y = proportion.sign.lrt.condition * 100)) +
  geom_col() +
...
ggsave(plot.sample.sizes.power.snuffle.mat, filename = "./plots/sample_sizes_power_analysis_snuffle_mat.png", scale = 0.55, height = 6, width = 15)
```
