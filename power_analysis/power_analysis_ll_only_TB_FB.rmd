...
n.blocks <- 3                                    # 3 blocks
conditions <- c("false_belief", "true_belief")
    ...
    sim_fun <- function(i) {
      ...
      full <- keepWarnings(glmer(
        choice ~ condition + sex + baited.fst + z.age +z.trial+
          (1 + condition.dummy1.c + z.trial || subj.id),
...
# Save results
save.image(file = here("power_analysis", "power_sim_snuffle_mat_workspace_parallel_10_percent_TB_FB_only.RData"))
```
...
```{r}
load(file = here("power_analysis", "power_sim_snuffle_mat_workspace_parallel_10_percent_TB_FB_only.RData"))
```
...
lrt.data2 <- all.res2 %>%
  dplyr::ungroup() %>% 
  dplyr::group_by(N, fb_per, tb_per) %>% 
  dplyr::summarise(
...
lrt.data2
#save.image("power_sim_snuffle_mat_workspace_parallel_10_percent_TB_FB_only_eval.RData")
...
ggsave(plot.sample.sizes.power.snuffle.mat, filename = "./plots/sample_sizes_power_analysis_snuffle_mat_10_percent_only_TB_FB.png", scale = 0.55, height = 6, width = 15)
```
